...
在动漫花园（`share.dmhy.org`）资源列表页的**每条资源旁**加「纸鸢」（转存）与「复制」两个按钮：前者通过纸鸢网盘的 **MCP（Streamable HTTP）** 接口把该资源的磁力链接转存到纸鸢网盘，后者把规范化后的磁力链接复制到剪贴板。
...
3. 打开 `https://share.dmhy.org/topics/list?keyword=红猪`，每条资源旁会出现「纸鸢」与「复制」按钮。

**脚本内不含任何 Token**——Token 属于隐私凭据，只保存在本机脚本存储中。首次使用时点击任意「纸鸢」按钮，按弹出框提示填写即可（Token 在纸鸢网盘 MCP 页面生成，只能明文查看一次）。「复制」按钮不需要 Token，开箱即用。

## 使用

| 操作 | 说明 |
|------|------|
| 首次使用 | 点击任意「纸鸢」按钮会弹出**图形化设置面板**，填入 MCP Token 并保存后自动重试转存 |
| 点击「纸鸢」 | 取出该行磁力链接并调用 `magnet_task_add`，成功后按钮变绿并显示任务 ID |
| 点击「复制」 | 把该行磁力链接（Base32 自动转 40 位 hex，与转存提交的一致）写入剪贴板，按钮短暂显示「已复制」；无需配置 Token |
| 按钮变红「重试转存」 | 点击可重新提交（服务端临时错误、网络抖动等） |
| 行内没有磁力图标 | 转存与复制都会自动回退到该资源的详情页解析磁力链接 |
...
该 mock 与真实 dmhy 的 `#topic_list` **同构**（含 `table-layout:fixed`、`td{overflow:hidden}`、`nowrap` 的「磁鏈」列及其中的磁力 / 迅雷 / PikPak 三个按钮）。改动注入位置相关的代码后，务必在浏览器里确认这三列原有按钮**没有被挤出或裁切**——jsdom 不做布局，这类缺陷只有真实浏览器才能发现。
...
测试会用 jsdom 加载并执行真实的 `dmhy-kiteyuan.user.js`（注入 `GM_*` 桩），覆盖 65 项断言：
按钮注入位置与数量、**按钮挂在标题列且不侵入「磁鏈」列**、行内磁力转存、成功后展示任务 ID、无磁力行回退详情页、重复磁力按「已存在」处理、**Base32 磁力自动转 hex 并去掉 tr 参数**、保留 tracker 开关、哈希非法时的排查提示、SSE 解析、握手回退、邀请码/Token 失效/业务错误/深层 error 提示、未配置 Token 时弹出设置面板并取消/保存/自动重试、重复点击不重复提交、失败后可重试、仅 GM4 风格 API 时的兼容路径、**复制按钮**（按钮顺序与文案、行内磁力直复制、无磁力行回退详情页后复制、Base32 转 hex 复制、状态复位、复制不触发任何转存任务、仅 `GM.setClipboard` 时的兼容分支、无任何剪贴板 API 时的降级失败提示）等。
...
**为什么按钮挂在标题列，而不是紧贴「磁力」图标？**
因为「磁鏈」列塞不下。真实 dmhy 的表格是 `table.tablesorter{table-layout:fixed}` + `width:100%`，该列列宽由表头 `width="10%"` 固定，完全不受内容影响，且该列是 `nowrap`、单元格 `overflow:hidden`。而这一列里 dmhy 自己已放了三个按钮（磁力图标、迅雷、PikPak），实测 1920 视口下 194px 的列宽已被它们占满，**零余量**。把文本按钮插进去会溢出并被裁切，把排在末尾的迅雷 / PikPak 按钮挤出可视区。

标题列（`td.title`）是唯一既宽（1920 视口下 819px）又允许换行（`word-break:break-all`，非 `nowrap`）的列，因此两个按钮作为一组挂在标题列末尾，换行时只增加约 1px 行高，不会挤掉任何原有元素。无磁力图标时（少数只有种子的资源）同样挂在标题列。

**点「复制」没反应 / 提示复制失败？**
脚本按 `GM_setClipboard`（或 GM4 风格的 `GM.setClipboard`）→ `navigator.clipboard` → `execCommand` 的顺序尝试写入剪贴板。若三者都被拒绝，请确认脚本管理器已授予剪贴板权限、页面处于聚焦状态（未最小化）；仍失败时可手动选中磁力链接复制。
...
- 「复制」功能会用到剪贴板权限（`@grant GM_setClipboard`，同时兼容 GM4 风格的 `GM.setClipboard`，并回退到 `navigator.clipboard` / `execCommand`）。复制内容只写入本机剪贴板，不会发往任何服务器。
